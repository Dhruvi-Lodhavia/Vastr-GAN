...
def parse_args():
    ...
    parser.add_argument('--cfg', dest='cfg_file',
                        ...
                        default='/content/drive/Shareddrives/ML: DF GAN for Fashion/DF-GAN/code/cfg/bird.yml', type=str)
...
def train(dataloader,netG,netD,text_encoder,optimizerG,optimizerD,state_epoch,batch_size,device):
    for epoch in range(state_epoch+1, cfg.TRAIN.MAX_EPOCH+1):
        ...
        #   os.mairs('/content/drive/Shareddrives/ML: DF GAN for Fashion/DF-GAN/imgs')
        vutils.save_image(fake.data,
                        '%s/fake_samples_epoch_%03d.png' % ('/content/drive/Shareddrives/ML: DF GAN for Fashion/DF-GAN/imgs', epoch),
                        normalize=True)
        if not os.path.exists('models'):
          os.makedirs('models')
        if not os.path.exists('models/bird'):
          os.makedirs('models/bird')
        # if epoch%10==0:
        torch.save({
            'epoch': epoch,
            'model_state_dict': netG.state_dict(),
            'optimizer_state_dict': optimizerG.state_dict(),
            }, '/content/drive/Shareddrives/ML: DF GAN for Fashion/DF-GAN/models/%s/netG_%03d.pth' % (cfg.CONFIG_NAME, epoch))
        torch.save({
            'epoch': epoch,
            'model_state_dict': netD.state_dict(),
            'optimizer_state_dict': optimizerD.state_dict(),
            }, '/content/drive/Shareddrives/ML: DF GAN for Fashion/DF-GAN/models/%s/netD_%03d.pth' % (cfg.CONFIG_NAME, epoch))
...
if __name__ == "__main__":
    ...
    checkpoint = torch.load("/content/drive/Shareddrives/ML: DF GAN for Fashion/DF-GAN/models/bird/netG_500.pth")
    netG.load_state_dict(checkpoint['model_state_dict'])
    optimizerG.load_state_dict(checkpoint['optimizer_state_dict'])
    state_epoch = checkpoint['epoch']
    checkpoint2 = torch.load("/content/drive/Shareddrives/ML: DF GAN for Fashion/DF-GAN/models/bird/netD_500.pth")
